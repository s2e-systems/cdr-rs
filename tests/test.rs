...
use cdr::{Bounded, Infinite, RepresentationFormat, Result, Error};

use serde_derive::{Deserialize, Serialize};

const ENCAPSULATION_HEADER_SIZE: usize = 4;

fn check<'de, T>(element: T, maybe_size: Option<usize>)
where
...
fn check_serialized_size<'de, T>(element: &T, maybe_size: Option<usize>)
where
    T: serde::Serialize + serde::Deserialize<'de> + PartialEq + Debug,
{
...
fn check_round_trip<'de, T>(element: &T, maybe_size: Option<usize>)
where
    T: serde::Serialize + serde::Deserialize<'de> + PartialEq + Debug,
{
    ...
    {
        let encoded =
            cdr::ser::serialize_data(element, RepresentationFormat::CdrBe, Infinite).unwrap();
        let decoded =
            cdr::de::deserialize_data::<T>(&encoded, RepresentationFormat::CdrBe).unwrap();
    ...
    {
        let encoded =
            cdr::ser::serialize_data(element, RepresentationFormat::CdrLe, Infinite).unwrap();
        let decoded =
            cdr::de::deserialize_data::<T>(&encoded, RepresentationFormat::CdrLe).unwrap();

        assert_eq!(*element, decoded);
        assert_eq!(size, encoded.len());
    }
    {
        let encoded = cdr::serialize(element, RepresentationFormat::CdrBe, Infinite).unwrap();
        let decoded = cdr::deserialize(&encoded).unwrap();

        assert_eq!(*element, decoded);
        assert_eq!(size + ENCAPSULATION_HEADER_SIZE, encoded.len());
    }
    {
        let encoded = cdr::serialize(element, RepresentationFormat::CdrLe, Infinite).unwrap();
        let decoded = cdr::deserialize(&encoded).unwrap();
    ...
    {
        let encoded = cdr::serialize(element, RepresentationFormat::PlCdrBe, Infinite).unwrap();
        let decoded = cdr::deserialize(&encoded).unwrap();
    ...
    {
        let encoded = cdr::serialize(element, RepresentationFormat::PlCdrLe, Infinite).unwrap();
        let decoded = cdr::deserialize(&encoded).unwrap();
...
fn check_capacity_shortage<'de, T>(element: &T, maybe_size: Option<usize>)
where
    T: serde::Serialize + serde::Deserialize<'de> + PartialEq + Debug,
{
    ...
    if let Some(bound) = calc_invalid_size(element, maybe_size) {
        {
            ...
            assert!(cdr::ser::serialize_data_into(
                &mut buf,
                &element,
        ...
        }
        {
            let mut buf = Cursor::new(&mut buf[0..bound as usize]);
            assert!(
                cdr::serialize_into(&mut buf, &element, RepresentationFormat::CdrBe, Infinite)
                    .is_err()
            );
        }
        {
            let mut buf = Cursor::new(&mut buf[0..bound as usize]);
            assert!(
                cdr::serialize_into(&mut buf, &element, RepresentationFormat::CdrLe, Infinite)
                    .is_err()
            );
        }
        {
            let mut buf = Cursor::new(&mut buf[0..bound as usize]);
            assert!(cdr::serialize_into(
                &mut buf,
                &element,
...
fn check_size_limit<'de, T>(element: &T, maybe_size: Option<usize>)
where
    T: serde::Serialize + serde::Deserialize<'de> + PartialEq + Debug,
{
    if let Some(bound) = calc_invalid_size(element, maybe_size) {
        assert!(
            cdr::ser::serialize_data(&element, RepresentationFormat::CdrBe, Bounded(bound))
                .is_err()
        );
        assert!(
            cdr::ser::serialize_data(&element, RepresentationFormat::CdrLe, Bounded(bound))
                .is_err()
        );
        assert!(cdr::serialize(&element, RepresentationFormat::CdrBe, Bounded(bound)).is_err());
        assert!(cdr::serialize(&element, RepresentationFormat::CdrLe, Bounded(bound)).is_err());
        assert!(cdr::serialize(&element, RepresentationFormat::PlCdrBe, Bounded(bound)).is_err());
        assert!(cdr::serialize(&element, RepresentationFormat::PlCdrLe, Bounded(bound)).is_err());
        {
            let encoded =
                cdr::ser::serialize_data(&element, RepresentationFormat::CdrBe, Infinite).unwrap();
            let mut encoded = encoded.as_slice();
            assert!(cdr::de::deserialize_data_from::<_,T,_>(
                &mut encoded,
                RepresentationFormat::PlCdrBe,
                Bounded(bound)
            )
        ...
        }
        {
            ...
            let mut encoded = encoded.as_slice();
            assert!(cdr::de::deserialize_data_from::<_, T, _>(
                &mut encoded, RepresentationFormat::CdrLe,
        ...
        {
            let encoded =
                cdr::ser::serialize_data(&element, RepresentationFormat::CdrBe, Bounded(0)).unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
                cdr::de::deserialize_data_from::<_, T, _>(&mut encoded, RepresentationFormat::CdrBe, Bounded(0))
                    .unwrap();
        ...
        {
            let encoded =
                cdr::ser::serialize_data(&element, RepresentationFormat::CdrLe, Bounded(0)).unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
                cdr::de::deserialize_data_from::<_, T, _>(&mut encoded, RepresentationFormat::CdrLe, Bounded(0))
                    .unwrap();
        ...
        {
            let encoded =
                cdr::serialize(&element, RepresentationFormat::CdrBe, Bounded(ENCAPSULATION_HEADER_SIZE))
                    .unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
        ...
        {
            let encoded =
                cdr::serialize(&element, RepresentationFormat::CdrLe, Bounded(ENCAPSULATION_HEADER_SIZE))
                    .unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
                cdr::deserialize_from::<_, T, _>(&mut encoded, Bounded(ENCAPSULATION_HEADER_SIZE))
        ...
        {
            let encoded =
                cdr::serialize(&element, RepresentationFormat::PlCdrBe, Bounded(ENCAPSULATION_HEADER_SIZE))
                    .unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
                cdr::deserialize_from::<_, T, _>(&mut encoded, Bounded(ENCAPSULATION_HEADER_SIZE))
        ...
        {
            let encoded =
                cdr::serialize(&element, RepresentationFormat::PlCdrLe, Bounded(ENCAPSULATION_HEADER_SIZE))
                    .unwrap();
            let mut encoded = encoded.as_slice();
            let decoded =
                cdr::deserialize_from::<_, T, _>(&mut encoded, Bounded(ENCAPSULATION_HEADER_SIZE))
...
fn calc_invalid_size<'de, T>(element: &T, maybe_size: Option<usize>) -> Option<usize>
where
    T: serde::Serialize + serde::Deserialize<'de> + PartialEq + Debug,
{
    match maybe_size {
...
fn test_unsupported() {
    ...
    check_error_kind(cdr::ser::serialize_data(
        &Some(1usize), RepresentationFormat::CdrBe,
        
        Infinite,
    ));
    check_error_kind(cdr::ser::serialize_data(
        &None::<usize>, RepresentationFormat::CdrBe,
        Infinite,
    ));

    check_error_kind(cdr::de::deserialize_data::<Option<usize>>(
        &Vec::new().as_slice(), RepresentationFormat::CdrBe
    ));
}
